...
public class DatasetGeneratorImpl implements DatasetGenerator {
	private static final String PORT = "/dev/tty/ACM0";
	private static final int RATE = 9600;
	
	private Optional<CommChannel> arduinoConnection;
	...
	public DatasetGeneratorImpl() {
		arduinoConnection = Optional.empty();
		try {
			arduinoConnection = Optional.of(new SerialCommChannel(PORT, RATE));
		} catch (Exception e) {
	...
	@Override
	public DynamicTimeSeriesCollection getData() {
		 final DynamicTimeSeriesCollection dataset =
				 new DynamicTimeSeriesCollection(1, COUNT, new Second());
		  dataset.setTimeBase(new Second());
	      dataset.addSeries(gaussianData(), 0, "Object's speed");
	      return dataset;
	}
